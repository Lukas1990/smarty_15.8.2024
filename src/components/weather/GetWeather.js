...
function GetWeather(props) {
   ...
   function getData() {
      const option_ele = select.current.selectedOptions[0]
      city.current.innerText = option_ele.innerText

      const latitude = option_ele.dataset.latitude
      const longitude = option_ele.dataset.longitude
  ...
  return (
   <div>
    ...
    {" "}<button onClick={getData} type="submit" className="btn">{phrase["Find out"]}</button>
  </div>
  );
}
